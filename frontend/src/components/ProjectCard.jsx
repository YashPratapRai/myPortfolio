...
/* ── Inject styles ONCE at module level — not inside render ── */
const STYLES = `
  ...
  .pc-card {
    ...
    /* removed inset box-shadow — GPU-expensive on many cards */
    box-shadow: 0 10px 40px -15px rgba(0,212,255,0.25);
    transition: border-color 0.3s ease, box-shadow 0.3s ease;
    /* tell browser to composite this layer */
    will-change: transform;
    cursor: default;
  }
  .pc-card.hovered {
    border-color: rgba(0,212,255,0.55);
    box-shadow: 0 20px 50px -10px rgba(0,212,255,0.35);
  }
...
/* Inject once into <head> — never repeated across card instances */
...
/* Stable particle positions — computed once per card mount, not on every render */
const PARTICLE_CONFIGS = Array.from({ length: 4 }, (_, i) => ({
  tx: Math.round(Math.random() * 160 - 80),
  ty: Math.round(Math.random() * 100 - 100),
  initX: Math.round(Math.random() * 300),
  animX: Math.round(Math.random() * 300),
  animY: Math.round(200 + Math.random() * 100),
...
const ProjectCard = ({
  ...
  /* Stable particle configs per card instance — won't change on re-render */
    ...
    <div className="pc-wrap">
      ...
      >
        ...
        <div className="pc-actions">
          {githubLink && (
            <a className="pc-btn" href={githubLink} target="_blank" rel="noopener noreferrer">
              <GitHubIcon /> Source
            </a>
          )}
          {demoUrl && (
            <a className="pc-btn" href={demoUrl} target="_blank" rel="noopener noreferrer">
              <LaunchIcon /> Live Demo
            </a>
          )}
        </div>

        {/* Particles — only rendered on hover, using stable pre-computed positions */}
        <AnimatePresence>
          {isHovered && particles.map((p, i) => (
            <motion.div
              key={i}
              className="pc-particle"
              initial={{ opacity: 0, x: p.initX, y: 350 }}
              animate={{ opacity: [0, 0.7, 0], x: p.animX + p.tx, y: p.animY }}
              exit={{ opacity: 0 }}
